...
library GoogleProtobufTimestamp {
  ...
  function _encode(
    ...
    if (r.data_seconds != 0) {
      ...
      pointer += ProtoBufRuntime._encode_int64(r.data_seconds, pointer, bs);
    }
    if (r.nanos != 0) {
      pointer += ProtoBufRuntime._encode_key(
        2,
  ...
  function _estimate(Data memory r) internal pure returns (uint256) {
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_int64(r.data_seconds);
  ...
  function store(Data memory input, Data storage output) internal {
    output.data_seconds = input.data_seconds;
...
//library AgoogleProtobufTimestamp
